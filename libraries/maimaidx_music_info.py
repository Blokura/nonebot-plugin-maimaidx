...
async def draw_music_info_to_message_segment(music: Music) -> MessageSegment:
    from nonebot.adapters.qq import MessageSegment
    res = await draw_music_info(music)
    res = save_image_file(res,'music_info_' + str(music.id))
    print(res)
    return MessageSegment.image(url=res)
...
async def music_play_data(username: str, songs: str) -> Union[str, MessageSegment, None]:
    """
    谱面游玩
    """
    try:
        version = list(set(_v for _v in plate_to_version.values()))
        data = await maiApi.query_user('plate', username=username, version=version)
        ...
        res = save_image_file(image_to_base64(im),'minfo_' + username)
        print(res)
...
async def new_draw_music_info(music: Music) -> str:
    ...
    res = save_image_file(image_to_base64(im),'music_info_' + str(music.id)+"_"+str(num))
    print(res)
    msg = MessageSegment.image(url=res)
