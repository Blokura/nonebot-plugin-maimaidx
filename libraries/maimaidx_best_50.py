...
async def generate(username: Optional[str] = None) -> str:
    try:
        ...
        obj = await maiApi.query_user('player', qqid=None, username=username)

        mai_info = UserInfo(**obj)
        draw_best = DrawBest(mai_info, 3889005734)
        
        pic = await draw_best.draw()

        res = save_image_file(image_to_base64(pic,format='JPEG'),'b50_' + username[0],'jpg')
        print(res)
